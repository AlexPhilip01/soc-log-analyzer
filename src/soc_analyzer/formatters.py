...
def format_pretty(events: list[SecurityEvent], use_color: bool = True, verbose: bool = False) -> str:
    lines: list[str] = []
    w = 60
...
def format_events(
    ...
    if fmt == "markdown":
        return format_markdown(events)
    return format_pretty(events, use_color=use_color, verbose=verbose)
