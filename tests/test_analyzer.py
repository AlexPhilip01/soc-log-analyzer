...
class TestWatcher:
    def test_file_tailer_reads_new_lines(self, tmp_path):
        from soc_analyzer.watcher import FileTailer
        log = tmp_path / "test.log"
        log.write_text("existing line\n")
        tailer = FileTailer(log)
        # Append new lines
        with open(log, "a") as fh:
            fh.write("May  1 03:21:04 host sshd[1]: Failed password for root from 1.2.3.4 port 22 ssh2\n")
            fh.write("May  1 03:21:05 host sshd[1]: Invalid user admin from 5.6.7.8 port 22\n")
        new_lines = tailer.read_new_lines()
        tailer.close()
        assert len(new_lines) == 2
        assert "Failed password" in new_lines[0]

    def test_file_tailer_handles_missing_file(self, tmp_path):
        from soc_analyzer.watcher import FileTailer
        log = tmp_path / "test.log"
        log.write_text("")
        tailer = FileTailer(log)
    ...
    def test_detect_type_from_file(self, tmp_path):
        from soc_analyzer.watcher import _detect_type_from_file
        log = tmp_path / "auth.log"
        log.write_text("May  1 03:21:04 host sshd[1]: Failed password for root from 1.2.3.4 port 22 ssh2\n")
        assert _detect_type_from_file(log) == "ssh"

    def test_parse_line_ssh(self):
        from soc_analyzer.watcher import _parse_line
        line = "May  1 03:21:04 host sshd[1]: Failed password for root from 1.2.3.4 port 22 ssh2"
        recs = _parse_line(line, "ssh", "test.log")
        assert len(recs) == 1
        assert recs[0]["log_type"] == "ssh"

    def test_parse_line_apache(self):
        from soc_analyzer.watcher import _parse_line
        line = '10.0.0.1 - - [01/May/2026:10:01:00 +0000] "GET /etc/passwd HTTP/1.1" 404 100 "-" "curl"'
        recs = _parse_line(line, "apache", "access.log")
        assert len(recs) == 1
        assert recs[0]["log_type"] == "apache"
...
class TestGeoIP:
    def test_private_ip_returns_private(self):
        from soc_analyzer.geoip import lookup_many, _PRIVATE_RESULT
        result = lookup_many(["192.168.1.1"])
        assert result["192.168.1.1"].is_private is True

    def test_loopback_is_private(self):
        from soc_analyzer.geoip import lookup_many
        result = lookup_many(["127.0.0.1"])
        assert result["127.0.0.1"].is_private is True

    def test_10_range_private(self):
        from soc_analyzer.geoip import lookup_many
        result = lookup_many(["10.0.0.1"])
        assert result["10.0.0.1"].is_private is True

    def test_172_16_private(self):
        from soc_analyzer.geoip import lookup_many
        result = lookup_many(["172.16.0.5"])
        assert result["172.16.0.5"].is_private is True

    def test_geo_result_short_private(self):
        from soc_analyzer.geoip import _PRIVATE_RESULT
        assert _PRIVATE_RESULT.short() == "Private / Internal"

    def test_geo_result_short_public(self):
        from soc_analyzer.geoip import GeoResult
        g = GeoResult(country="United States", country_code="US",
                      city="San Jose", org="AS13335 Cloudflare", is_private=False)
        s = g.short()
        assert "US" in s
        assert "San Jose" in s

    def test_enrich_events_no_geo(self):
        from soc_analyzer.geoip import enrich_events
        recs = parse_ssh(SSH_LINES)
        events = run_rules(recs)
        result = enrich_events(events, use_geo=False)
        assert result == {}

    def test_enrich_events_private_only(self):
        """Private IPs should be resolved without network call."""
        from soc_analyzer.geoip import enrich_events
        recs = parse_ssh(SSH_LINES)  # all IPs in SSH_LINES are private/RFC-1918
        events = run_rules(recs)
        geo = enrich_events(events, use_geo=True)
        for ip, result in geo.items():
            if ip.startswith("192.168.") or ip.startswith("10.") or ip.startswith("172."):
                assert result.is_private is True

    def test_cache_is_used(self):
        """Calling lookup_many twice should hit cache on second call."""
        from soc_analyzer import geoip
        geoip._CACHE.clear()
        ip = "192.168.99.99"
        geoip.lookup_many([ip])
        assert ip in geoip._CACHE
        # Second call should return from cache
        result2 = geoip.lookup_many([ip])
        assert result2[ip].is_private is True


# ──────────────────────────────────────────────────────────────────────────────
# Alerting / webhook tests
# ──────────────────────────────────────────────────────────────────────────────

class TestAlerting:
    def _make_event(self, sev=Severity.CRITICAL):
    ...
    def test_alerter_skips_below_threshold(self):
        from soc_analyzer.alerting import WebhookAlerter
        alerter = WebhookAlerter(url="http://localhost:9999/noop",
                                 min_severity=Severity.CRITICAL, quiet=True)
        ev = self._make_event(Severity.LOW)
        alerter(ev)  # should not attempt network call
        assert alerter.stats == (0, 0)

    def test_send_batch_counts_qualifying(self):
        """Batch send: only HIGH+ events are counted as attempted."""
        from soc_analyzer.alerting import WebhookAlerter
        alerter = WebhookAlerter(url="http://localhost:1/noop",
                                 min_severity=Severity.HIGH, quiet=True)
        events = [
            self._make_event(Severity.INFO),
            self._make_event(Severity.LOW),
            self._make_event(Severity.MEDIUM),
        ]
        alerter.send_batch(events)
        # None qualify → 0 sent, 0 failed
        assert alerter.stats[0] == 0

    def test_slack_payload_structure(self):
        from soc_analyzer.alerting import _slack_payload
        ev = self._make_event()
        payload = _slack_payload(ev)
        assert "attachments" in payload
        assert "text" in payload
        assert "CRITICAL" in payload["text"]

    def test_generic_payload_structure(self):
        from soc_analyzer.alerting import _generic_payload
        ev = self._make_event()
        payload = _generic_payload(ev)
        assert "alert" in payload
        assert payload["alert"]["rule_id"] == "ssh_failed_login"
        assert payload["alert"]["severity"] == "CRITICAL"

    def test_is_slack_url(self):
        from soc_analyzer.alerting import _is_slack_url
        assert _is_slack_url("https://hooks.slack.com/services/T00/B00/xxx") is True
        assert _is_slack_url("https://my.webhook.example.com/alert") is False

    def test_webhook_fails_gracefully(self):
        """Unreachable URL should increment failed count, not raise."""
        from soc_analyzer.alerting import WebhookAlerter
        alerter = WebhookAlerter(url="http://localhost:1/dead",
                                 min_severity=Severity.CRITICAL, quiet=True)
        ev = self._make_event(Severity.CRITICAL)
        alerter(ev)
        assert alerter.stats[1] == 1   # 1 failed

    def test_cli_no_alert_webhook(self):
        """Running without --alert-webhook should not import WebhookAlerter issues."""
        auth_log = SAMPLE_DIR / "auth.log"
        if not auth_log.exists():
            pytest.skip("Sample file not found")
        rc = main([str(auth_log), "--no-color"])
